<template>
    <div class="container">
        <h3 class="h1" style="color:red !important;padding-left:30px">Latest Politics News</h3>
        <div class="col-lg-12 col-md-12 col-sm-12">
            <ul class="list-unstyled" style="font-size:13px">
            <li class="well well-sm" style="height:100px">
                <a href="#">
                    <img src="img/news/1.jpg" width="50px" height="50px" align="left" style="border-radius:50px">Call for change in the UNEP
                </a>
                <br>
                UN climate change talks in Madrid are struggling to reach agreement on crucial measures<br>
                <span style="font-weight:bold;opacity:.4">Category:</span><i><b>Science & Environment</b></i>
            </li>
            <li class="well well-sm" style="height:100px">
                <a href="#">
                    <img src="img/news/4.jpg" width="50px" height="50px" align="left" style="border-radius:50px">Call for change in the UNEP
                </a>
                <br>
                UN climate change talks in Madrid are struggling to reach agreement on crucial measures.<br>
                <span style="font-weight:bold;opacity:.4">Category:</span><i><b>Science & Environment</b></i>
            </li>
            <li class="well well-sm" style="height:100px">
                <a href="#">
                    <img src="img/news/1.jpg" width="50px" height="50px" align="left" style="border-radius:50px">Call for change in the UNEP
                </a>
                <br>
                UN climate change talks in Madrid are struggling to reach agreement on crucial measures<br>
                <span style="font-weight:bold;opacity:.4">Category:</span><i><b>Science & Environment</b></i>
            </li>
            <li class="well well-sm" style="height:100px">
                <a href="#">
                    <img src="img/news/4.jpg" width="50px" height="50px" align="left" style="border-radius:50px">Call for change in the UNEP
                </a>
                <br>
                UN climate change talks in Madrid are struggling to reach agreement on crucial measures.<br>
                <span style="font-weight:bold;opacity:.4">Category:</span><i><b>Science & Environment</b></i>
            </li>
            <li class="well well-sm" style="height:100px">
                <a href="#">
                    <img src="img/news/1.jpg" width="50px" height="50px" align="left" style="border-radius:50px">Call for change in the UNEP
                </a>
                <br>
                UN climate change talks in Madrid are struggling to reach agreement on crucial measures<br>
                <span style="font-weight:bold;opacity:.4">Category:</span><i><b>Science & Environment</b></i>
            </li>
            <li class="well well-sm" style="height:100px">
                <a href="#">
                    <img src="img/news/4.jpg" width="50px" height="50px" align="left" style="border-radius:50px">Call for change in the UNEP
                </a>
                <br>
                UN climate change talks in Madrid are struggling to reach agreement on crucial measures.<br>
                <span style="font-weight:bold;opacity:.4">Category:</span><i><b>Science & Environment</b></i>
            </li>
            <li class="text-center">
                <a href="#" class="btn"><i class="fa fa-angle-double-down"></i>More News</a>
            </li>
        </ul>
        </div>
    </div>
</template>